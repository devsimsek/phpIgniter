 ...
 *  Database.php File
 *
 *  This driver contains functions that will integrate database to phpIgniter
 *
 *  @file         Database.php
 *  @package      phpIgniter Core
 *  @author       Metin Şimşek (mtnsmsk) <mtnsmsk@smsk.me, smsk.me>
 *  @link         https://smsk.me/phpigniter/docs/Database.php
 *
 * ------------------------------------------------------------------------
 ******************************************************************************/
...
class Database
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            show_error('Database Error Occurred', $e->getMessage(), 'error_db', 500);
        }
    }
    ...
    public function _max_min_sum_count_avg($column, $alias = null, $type = 'MAX')
    {
        if (!in_array($type, array('MAX', 'MIN', 'SUM', 'COUNT', 'AVG'))) {
            show_error('Database Error Occurred', 'Invalid function type: ' . html_escape($type), 'error_db', 500);
        }
    ...
    public function get($mode = PDO::FETCH_ASSOC)
    {
        ...
        try {
            ...
            show_error('Database Error Occurred', $e->getMessage() . '<br>SQL Query: ' . html_escape($this->getSQL), 'error_db', 500);
        }
    }
